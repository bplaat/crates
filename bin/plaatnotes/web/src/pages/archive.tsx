...
export function ArchivePage() {
    ...
    // @ts-ignore
    useEffect(async () => {
        document.title = 'PlaatNotes - Archive';
        const data = await listNotes();
        setNotes(data.filter((n) => n.isArchived && !n.isTrashed));
        setLoading(false);
    }, []);
    ...
    return (
        <Layout>
            <div class="max-w-screen-xl mx-auto px-4 py-6">
                <h1 class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-6">Archive</h1>
                ...
                {!loading && notes.length === 0 && (
                    <div class="flex flex-col items-center justify-center mt-24 gap-3 text-gray-400">
                        ...
                        <p class="text-lg">No archived notes.</p>
                    </div>
                )}
