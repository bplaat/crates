...
- (void)applicationDidFinishLaunching:(NSNotification*)notification {
    ...
    [NSTimer scheduledTimerWithTimeInterval:30 * 60
                                     target:self
                                   selector:@selector(updateStreakTimer:)
                                   userInfo:nil
                                    repeats:YES];
    ...
    if (self.userId != 0) {
        [menu addItemWithTitle:NSLocalizedString(@"Loading streak...", @"Menu streak loading")
                        action:nil
                 keyEquivalent:@""];
    } else {
        [menu addItemWithTitle:NSLocalizedString(@"No username set...", @"Menu no username")
                        action:nil
                 keyEquivalent:@""];
    }
    [menu addItem:[NSMenuItem separatorItem]];
    [menu addItemWithTitle:NSLocalizedString(@"Settings", @"Settings")
                    action:@selector(openSettings:)
             keyEquivalent:@","];
    [menu addItem:[NSMenuItem separatorItem]];
    [menu addItemWithTitle:NSLocalizedString(@"About XStreaks", @"Menu about")
                    action:@selector(openAbout:)
             keyEquivalent:@""];
    [menu addItemWithTitle:NSLocalizedString(@"Quit XStreaks", @"Menu quit")
        ...
        if (streakDays == 1) {
            [firstMenuItem
                setTitle:[NSString stringWithFormat:
                                       NSLocalizedString(
                                           @"Current streak: %ld day",
                                           @"Menu streak single ('%ld' is a placeholder where the number is put)"),
                                       streakDays]];
        } else {
            [firstMenuItem
                setTitle:[NSString stringWithFormat:
                                       NSLocalizedString(
                                           @"Current streak: %ld days",
                                           @"Menu streak plural ('%ld' is a placeholder where the number is put)"),
                                       streakDays]];
        }
...
- (void)updateUsername:(NSString*)username {
    ...
    NSURL* url =
        [NSURL URLWithString:[NSString stringWithFormat:@"https://api.x.com/2/users/by/username/%@", encodedUsername]];
...
- (void)updateStreak:(bool)userInitiated {
    ...
    NSURL* url = [NSURL
        URLWithString:
            [NSString stringWithFormat:@"https://api.x.com/2/users/%ld/tweets?max_results=100&tweet.fields=created_at",
                                       (long)self.userId]];
    NSLog(@"[INFO] Fetching %@...", url);
    NSMutableURLRequest* request = [NSMutableURLRequest requestWithURL:url];
    [request setValue:[NSString stringWithFormat:@"Bearer %s", API_KEY] forHTTPHeaderField:@"Authorization"];
...
- (void)openToManyRequestsAlert {
    ...
    [alert setInformativeText:NSLocalizedString(@"You have made too many requests. Please try again later.",
