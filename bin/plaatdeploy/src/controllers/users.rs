...
use super::{parse_body, parse_body_ctx, parse_pagination, require_auth};
use crate::api;
use crate::context::{Context, DatabaseHelpers};
use crate::models::{User, UserRole};
use crate::utils::password_hash;

fn validate_unique_email(email: &str, ctx: &Context) -> validate::Result {
    let count = ctx
        .database
        .query_some::<i64>(
        ...
        .map_err(|_| validate::Error::new("Could not validate email"))?;
    if count != 0 {
        return Err(validate::Error::new("Email is already taken"));
    }
    Ok(())
}

struct UserUpdateContext {
    ctx: Context,
    user_id: Uuid,
}

impl Default for UserUpdateContext {
    fn default() -> Self {
        unreachable!("UserUpdateContext must be supplied explicitly")
    }
}

fn validate_unique_email_for_update(email: &str, ctx: &UserUpdateContext) -> validate::Result {
    if let Some(existing_user) = ctx
        .ctx
        .database
        .find_user_by_email(email)
        .map_err(|_| validate::Error::new("Could not validate email"))?
        && existing_user.id != ctx.user_id
    {
        return Err(validate::Error::new("Email is already taken"));
    }
    Ok(())
...
pub(crate) fn users_create(req: &Request, ctx: &Context) -> Result<Response> {
    ...
    struct Body {
        ...
        password: String,
        role: Option<api::UserRole>,
    }

    let body = parse_body_ctx!(req, api::UserCreateBody, Body, ctx);
    ...
    let user = User {
        ...
        role: body.role.map(Into::into).unwrap_or(UserRole::Normal),
        ..Default::default()
    };
...
pub(crate) fn users_update(req: &Request, ctx: &Context) -> Result<Response> {
    ...
    #[validate(context(UserUpdateContext))]
    struct Body {
        #[validate(length(min = 1, max = 128))]
        first_name: String,
        #[validate(length(min = 1, max = 128))]
        last_name: String,
        #[validate(email, custom(validate_unique_email_for_update))]
        email: String,
        #[validate(ascii, length(min = 8, max = 128))]
    ...
    {
        user.password = password_hash(&pw);
    }
    if auth_user.role == UserRole::Admin
        && let Some(role_str) = body.role
    {
        user.role = role_str.into();
    }
...
pub(crate) fn users_change_password(req: &Request, ctx: &Context) -> Result<Response> {
    ...
    #[derive(Validate, FromStruct)]
    #[from_struct(api::ChangePasswordBody)]
    struct Body {
        #[validate(length(min = 1, max = 128))]
        old_password: String,
        #[validate(ascii, length(min = 8, max = 128))]
    ...
    let body = parse_body!(req, api::ChangePasswordBody, Body);
