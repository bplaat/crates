...
mod tests {
    ...
    #[test]
    fn inflates_valid_gzip_data() {
        assert_eq!(
            inflate_gzip(HELLO_GZIP).as_deref(),
            Some(b"hello".as_slice())
        );
    }

    #[test]
    fn rejects_invalid_or_truncated_gzip_data() {
        assert_eq!(inflate_gzip(b"not gzip"), None);
        assert_eq!(inflate_gzip(&HELLO_GZIP[..12]), None);

        let mut wrong_method = HELLO_GZIP.to_vec();
        wrong_method[2] = 0;
