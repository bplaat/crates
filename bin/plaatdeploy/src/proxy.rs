...
pub(crate) fn dispatch(
    ...
    } else if let Some(subdomain) = host.strip_suffix(&format!(".{}", ctx.deployments_host)) {
...
fn serve_static(req: &Request, project: &Project, deploy_path: &str) -> Response {
    if !validate_project_name(&project.name)
        || normalize_base_dir(&project.base_dir).as_deref() != Some(project.base_dir.as_str())
    {
        return Response::new()
            .status(Status::ServiceUnavailable)
            .body("503 Project path configuration invalid");
    }

    let repo = format!("{deploy_path}/{}/repo", project.name);
    let base = if project.base_dir.is_empty() {
        repo.clone()
    } else {
        format!("{repo}/{}", project.base_dir)
    };
    ...
    let canonical_file = match std::fs::canonicalize(file_path_obj) {
        ...
        Err(_) => {
            ...
            let canonical_fallback = match std::fs::canonicalize(&fallback) {
                Ok(path) => path,
                Err(_) => {
                    return Response::new()
                        .status(Status::NotFound)
            ...
            if !canonical_fallback.starts_with(&canonical_base) {
                return Response::new()
                    .status(Status::Forbidden)
                    .body("403 Forbidden");
            }
            match std::fs::read(&canonical_fallback) {
                Ok(data) => {
                    return Response::new()
...
fn project_container_port(project: &Project) -> Option<i64> {
    let port = project.container_port.unwrap_or(3000);
...
fn proxy_websocket(req: &Request, project: &Project) -> Response {
    ...
    let container_port = match project_container_port(project) {
        Some(port) => port,
        None => {
            return Response::new()
                .status(Status::ServiceUnavailable)
                .body("503 Project port configuration invalid");
        }
    };
