...
impl Sha1 {
    ...
    unsafe fn process_block_x86_sha(&mut self) {
        unsafe {
            ...
            // Load state: abcd = [a, b, c, d] reversed, e0 = [e, 0, 0, 0]
            let abcd = _mm_shuffle_epi32(
                _mm_loadu_si128(self.state.as_ptr().cast()),
                0x1B, // reverse word order
            );
            let e0 = _mm_set_epi32(self.state[4] as i32, 0, 0, 0);
            let abcd_save = abcd;
            let e0_save = e0;
