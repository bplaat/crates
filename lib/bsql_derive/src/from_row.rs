...
pub(crate) fn from_row_derive(input: TokenStream) -> TokenStream {
    ...
    // Parse fields and handle #[sql(skip)] and #[sql(rename = "example")] attributes
    ...
    let from_rows = fields
        ...
        .map(|(index, (field, field_name))| {
            let index = index as i32;
            let ident = field.ident.as_ref().expect("Invalid field");
            quote! { #ident: statement.column_value(#index).try_into().map_err(|_| bsql::ValueError::new(format!(
    ...
    TokenStream::from(quote! {
        ...
        impl #impl_generics bsql::Bind for #name #type_generics #where_clause {
            fn bind(self, statement: &mut bsql::RawStatement) -> Result<(), bsql::StatementError> {
                #( #binds )*
                Ok(())
            }
        }
        impl #impl_generics bsql::FromRow for #name #type_generics #where_clause {
            fn from_row(statement: &mut bsql::RawStatement) -> Result<Self, bsql::ValueError> {
                Ok(Self {
                    #( #from_rows, )*
                    #from_rows_default
                })
