...
use super::webview2::{
    ...
    ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandlerVtbl, ICoreWebView2EnvironmentOptions,
...
pub(super) const WEBVIEW2_RUNTIME_NOT_FOUND: HRESULT =
...
fn find_evergreen_runtimes() -> Vec<OsString> {
    ...
    for root in [HKEY_LOCAL_MACHINE, HKEY_CURRENT_USER] {
        ...
        if status == ERROR_SUCCESS {
            let len = path.iter().position(|&value| value == 0).unwrap_or(path.len());
            runtimes.push(OsString::from_wide(&path[..len]));
        }
    }
    runtimes
}
