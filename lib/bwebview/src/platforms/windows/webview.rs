            ...
            if result != S_OK {
                if result == WEBVIEW2_RUNTIME_NOT_FOUND {
                    if MessageBoxA(
                        ...
                        c"Microsoft Edge WebView2 Runtime is required to run this application.\n\nDownload it now?"
                            .as_ptr(),
                        c"WebView2 Runtime Required".as_ptr(),
                    ...
                    ) == IDYES
                    {
                        ShellExecuteA(
                            self.webview_data.hwnd,
                            c"open".as_ptr(),
                            c"https://developer.microsoft.com/microsoft-edge/webview2/#download-section"
                                .as_ptr(),
                            null(),
                            null(),
                            SW_SHOWNORMAL,
            ...
            ) == S_OK
            {
                ...
                if (*environment10).CreateCoreWebView2ControllerOptions(&mut options) == S_OK {
                    ...
                    ) == S_OK
                    {
                        ...
                        {
                            controller_creation_started = (*environment10)
