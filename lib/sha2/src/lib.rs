...
impl Sha256 {
    ...
    unsafe fn process_block_x86_sha(&mut self) {
        unsafe {
            ...
            // state0 = [a, b, e, f] (after shuffle), state1 = [c, d, g, h] (after shuffle)
            let tmp = _mm_loadu_si128(self.state.as_ptr().cast()); // [a, b, c, d]
            let tmp2 = _mm_loadu_si128(self.state.as_ptr().add(4).cast()); // [e, f, g, h]
            let mut state0 = _mm_shuffle_epi32(_mm_unpacklo_epi32(tmp, tmp2), 0xD8); // [a, e, b, f] -> [a, b, e, f]
            let mut state1 = _mm_shuffle_epi32(_mm_unpackhi_epi32(tmp, tmp2), 0xD8); // [c, g, d, h] -> [c, d, g, h]

            // Actually SHA-NI expects cdgh in state0 and abef in state1... let me swap
            // state0 = cdgh, state1 = abef
            std::mem::swap(&mut state0, &mut state1);

            let state0_save = state0;
            let state1_save = state1;

            // Load message words and byte-swap
            ...
            // Unshuffle state: state0 = cdgh, state1 = abef -> [a,b,c,d] and [e,f,g,h]
            let tmp = _mm_shuffle_epi32(_mm_unpacklo_epi32(state1, state0), 0xD8); // [a, c, b, d] -> [a, b, c, d]
            let tmp2 = _mm_shuffle_epi32(_mm_unpackhi_epi32(state1, state0), 0xD8); // [e, g, f, h] -> [e, f, g, h]
            _mm_storeu_si128(self.state.as_mut_ptr().cast(), tmp);
            _mm_storeu_si128(self.state.as_mut_ptr().add(4).cast(), tmp2);
        }
    }
